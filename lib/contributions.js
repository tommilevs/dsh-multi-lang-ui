...
function isSafeDomSelector(selector) {
  ...
  const root = /^(?:\.[A-Za-z_][\w-]*|\[data-dsh-plugin="[A-Za-z0-9][A-Za-z0-9._-]*"\]|\[data-conversation-tabs\]|\[data-dsh-pet-root\]|section\[data-plugin-panel\]|#settings-pet-[A-Za-z0-9_-]+|section\[aria-labelledby="(?:vision|compact|approve|subagent|title|imagegen)-title"\]|p\[role="alert"\])/.exec(value)
  ...
  if (pack.dom !== undefined) {
    if (!Array.isArray(pack.dom)) {
      ...
      pack.dom.forEach((entry, index) => {
        ...
        if (!isSafeDomSelector(entry.selector)) {
          issues.push(`${path}.selector must be a scoped CSS selector rooted at a plugin marker`)
        }

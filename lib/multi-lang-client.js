...
function applyMultiLang(ctx) {
  ...
  const isSafePack = (pack) => {
    ...
    if (pack.dom !== undefined) {
      ...
      if (pack.dom.some((entry) => !entry || typeof entry.selector !== 'string' || entry.selector.length > 300 || !/^\.[A-Za-z_][\w-]*(?:[\s>+~.#:[\]="'()\w-]*)$/.test(entry.selector.trim()) || /(?:^|[\s>+~])(?:body|html|\*)\b/i.test(entry.selector) || typeof entry.source !== 'string' || !entry.source.trim() || typeof entry.target !== 'string' || !entry.target.trim() || !validatePlaceholders(entry.source, entry.target))) return false
    }
    return true
  }
  const activeId = () => {
  ...
  const resolveText = (selector, text, language) => {
    const trimmed = text.trim()
    const targetFor = (entry) => entry.targets[language] || entry.targets[language.split('-')[0]]
    for (const entry of domIndex.values()) {
      if (entry.selector !== selector) continue
      if (entry.source === trimmed) return targetFor(entry) || entry.source
      for (const [lang, target] of Object.entries(entry.targets)) {
        if (target === trimmed) return language === lang || language.startsWith(`${lang}-`) ? target : (targetFor(entry) || entry.source)
      }
    }
    return text
  }
  ...
  const translateRoot = (root, language) => {
    ...
    for (const node of nodes) {
      ...
      const translated = resolveText(selector, node.nodeValue.trim(), language)
    ...
    for (const element of root.querySelectorAll?.('[title],[placeholder],[aria-label]') || []) {
      ...
      for (const attribute of ['title', 'placeholder', 'aria-label']) {
        ...
        if (!value) continue
        const translated = resolveText(selector, value.trim(), language)
  ...
  if (typeof fetch === 'function') {
    fetch(PACKS_URL, { headers: { Accept: 'application/json' } }).then((response) => response.ok ? response.json() : null).then((data) => {
